...
public class CastingChamberContainer extends Container {

    private CastingChamberTileEntity te;

    public CastingChamberContainer(IInventory playerInventory, CastingChamberTileEntity te) {
        this.te = te;
    ...
    private void addOwnSlots() {
        CastingChamberItemHandler itemHandler = (CastingChamberItemHandler)this.te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);


        // Add our own slots
        int slotIndex = 0;
        //add mold slot
        addSlotToContainer(new SlotItemHandler(itemHandler, slotIndex,37,35));
        slotIndex++;
        //add output slot
        addSlotToContainer(new PullOnlySlot(itemHandler, slotIndex, 87,35));
        slotIndex++;
        //add upgrade slots
        int y=8;
        for (int i = 2; i < itemHandler.getSlots(); i++) {
            addSlotToContainer(new SlotItemHandler(itemHandler, slotIndex, 152, y));
            slotIndex++;
            y += 18;
        }
    }
